...
public:
	WaveFunction(map<int, CV_state>* states) : Matrix() {
	...
	void set_amplitude(CV_state* state, double val) {
		for (auto k : *states_) {
			if (k.second == *state) {
				cout << k.first << endl;
				cpu_data_[k.first] = Complexd(1, 0).value();
	...
	double norm() {
		...
		for (int i = 0; i < M_; i++) {
			norm = norm + (get(i, 0).conj() * get(i, 0));
		}
