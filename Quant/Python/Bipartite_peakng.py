...
def get_period(filename='', make_plot=False, thres_up=thres_up, thres_down=thres_down, npoints=-1):
    ...
    if npoints == -1:
        ...
        data = pd_data['fidelity'].tolist()[0:npoints]
    ...
    for i in range(0, len(l) - 1):
        ...
        for m in keys:
            if(m > l[i] and m < l[i+1] and ind[m] == 'max'):
                if(data[m] > MAX_H):
                    # print("\t", m)
                    MAX = m
                    MAX_H = data[m]
    ...
    # print(ind)
    # print(keys)
    return np.mean(T) / 20000*1000


# def get_mean_peak(filename, make_plot=False):
#     if not os.path.isfile(filename):
#         return None

#     pd_data = pd.read_filename(filename)
#     # data = milk_data.iloc[:, 10]
...
#                          '/z_0.filename', make_plot=make_plot)
#     if data is None:
#         break
